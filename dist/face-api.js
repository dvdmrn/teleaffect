...
function L2norm(vec){
  let sumOfSquares = Math.pow(vec[0],2) + Math.pow(vec[1],2);
  ...
  ***************************************************************************** */
  ...
  var FaceLandmarks = /** @class */ (function () {
      function FaceLandmarks(relativeFaceLandmarkPositions, imgDims, shift) {
          ...
          // console.log(shift.x-250)
          // console.log(shift.y-150)
          socket.emit("positionalData",{"dist":mouthDist, "globalPos":shift, "eyeDist":eyeDist, "userID":socket.id});
      }
      ...
      Object.defineProperty(FaceLandmarks.prototype, "positions", {
          get: function () { 
            // console.log("global pos: ",this._positions)
      ...
      Object.defineProperty(FaceLandmarks.prototype, "relativePositions", {
          get: function () {
              var _this = this;
              let jawLineMag = L2norm(vectorSubtraction(this.positions[0],this.positions[16]));
              let lipsMag = L2norm(vectorSubtraction(this.positions[48],this.positions[64]));
              let eyeMag = L2norm(vectorSubtraction(this.positions[20],this.positions[38]));

              mouthDist = Math.pow((lipsMag/jawLineMag) * 100, 2)
              eyeDist = Math.pow((eyeMag/jawLineMag) * 10 ,6)
              // !!! important for finding relative positions... now need to figure out what each rel position means
              // console.log("normalized lip distance: ",Math.pow((lipsMag/jawLineMag) * 100, 2));
  ...
  var FaceLandmarks68 = /** @class */ (function (_super) {
    // !!! indexes for landmarks of interest
  ...
  var DrawFaceLandmarks = /** @class */ (function () {
      ...
      DrawFaceLandmarks.prototype.draw = function (canvasArg) {
          ...
          if (drawPoints) {
              ...
              var drawPoint = function (pt,ptdx) {
                // !!! checks landmark indx
                // 64:=rightmost
                // 48:=leftmost
                // JAW
                // 16:=rightmost
              ...
              // console.log(this.faceLandmarks.positions)
              for (var i = 0; i < this.faceLandmarks.positions.length; i++) {
                drawPoint(this.faceLandmarks.positions[i],i);
              }
